...
const displayCategory = (data) => {
    ...
    data.forEach(items => {
        ...
        div.innerHTML = `
    <button class="btn btn-lg flex gap-1 w-full items-center justify-center border border-zinc-400 h-[80px] object-contain category-btn" id="btn-${items.category.toLowerCase()}" onclick="loadPetByCategories('${items.category.toLowerCase()}')"><img  src=${items.category_icon} class="w-10"/> <p class="text-xl">${items.category}</p> </button>
    `
    ...
    <div class="card-body px-5">
        ...
        <div class="flex flex-col gap-2">
            <p><i class="fa-solid fa-border-all"></i> Breed: ${
              items?.breed ?? "Not Found"
            }</p>
            <p><i class="fa-solid fa-calendar-days"></i> Birth: ${
              items?.date_of_birth ?? "Not Available"
            }</p>
            <p><i class="fa-solid fa-venus"></i> Gender: ${
              items?.gender ?? "Not Found"
            }</p>
            <p><i class="fa-solid fa-dollar-sign"></i> Price: ${
              items?.price ?? "Not Define"
            }$</p>

        </div>
        <div class="flex justify-between">
            <button class="btn text-[#0e7a81] text-lg" onclick="likedPet('${
              items.image
            }')"><i class="fa-solid fa-thumbs-up"></i></button>
            <button class="btn text-[#0e7a81] text-lg" onclick="adoptionProcess()" id="adopt-button">Adopt</button>
            <button class="btn text-[#0e7a81] text-lg" onclick="showDetails('${
              items.petId
            }')">Details</button>
        </div>
    </div>
